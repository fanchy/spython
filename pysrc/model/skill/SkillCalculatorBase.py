...
def calPhysicHurt(obj):
    return random.randint(obj.physicAttackMin, obj.physicAttackMax)
...
def calPhysicDefend(obj):
    ...
    return random.randint(obj.physicDefendMin, obj.physicDefendMax) * (1 + addDefend)
#实际魔法攻击力=随机值（最小魔法攻击力，最大魔法攻击力）*（1+BUFF增加系数）
def calMagicHurt(obj):
     return random.randint(obj.magicAttackMin, obj.magicAttackMax)
...
def calMagicDefend(obj):
    ...
    if buff:
        ...
        addDefend += buff.param.get(1, 0)
    return random.randint(obj.magicDefendMin, obj.magicDefendMax) * (1 + addDefend)
# 战力计算
...
def calRealPhysicHurt(srcObj, destObj, skill, skillHurtCalculator, hitFlag = True, hpAbsorbFlag = True, ignoreDefaendFlag = 0, defendParam = 0):
    ...
    if ignoreDefaendFlag & CalHurtFlag.IGNORE_PHYSIC_DEFEND:
